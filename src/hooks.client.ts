  ...
  switch (e.data.action) {
    case 'chunkGifWorkerResult': {
      document.dispatchEvent(
        new CustomEvent(CropperEvent.CHUNK_GIF_RESULT, {
          detail: e.data.data
        })
      );
      break;
    ...
    case 'cropChunkWorkerResult': {
      document.dispatchEvent(
        new CustomEvent(CropperEvent.CROP_CHUNK_RESULT, {
          detail: {
            data: e.data.data,
            chunk: e.data.chunk
          }
    ...
    case 'mergeFramesWorkerResult': {
      document.dispatchEvent(
        new CustomEvent(CropperEvent.MERGE_FRAMES_RESULT, {
          detail: e.data.data
        })
      );
      break;
    ...
    case 'cropImageWorkerResult': {
      document.dispatchEvent(
        new CustomEvent(CropperEvent.CROP_IMAGE_RESULT, {
          detail: e.data.data
        })
      );
      break;
...
document.addEventListener('cropGifInWorker', (async (e: CropEvent) => {
  let data = e.detail;
  ...
  let croppedChunks: Uint8Array[];
  {
    ...
    for (let i = 0; i < workers.length; i++) {
      ...
      waiters.push(
        events.consumeSingleEvent(CropperEvent.CROP_CHUNK_RESULT, (e) => e.detail.chunk == i)
      );
    }
  ...
  let combined: MergeFramesResultEvent = await events.consumeSingleEvent(
    CropperEvent.MERGE_FRAMES_RESULT
  );
  document.dispatchEvent(
    new CustomEvent('cropGifResult', {
      detail: new Blob([combined.detail])
    })
  );
}) as events.AsyncEventListener);

document.addEventListener('cropImageInWorker', (async (e: CropEvent) => {
  let data = e.detail;
  ...
  document.dispatchEvent(
    new CustomEvent('cropImageResult', {
